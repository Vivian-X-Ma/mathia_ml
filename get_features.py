...
problem_features = pd.concat([
    hint_counts,
    hint_freq.rename('Hint_Request_Frequency'),
    time_per_problem.rename('Time_Per_Problem')
], axis=1).reset_index()
...
student_features['Total_Problems_Attempted'] = problem_features.groupby('Anon.Student.Id')['Problem.Name'].nunique()

# Total hint requests
student_features['Total_Hint_Requests'] = problem_features.groupby('Anon.Student.Id')[['Level1_Hints', 'Level2_Hints', 'Level3_Hints']].sum().sum(axis=1)
